# Infolitico Docs agent guide

## Memory routing

1. Read `.codex/harness-memory.json`; the project is `infolitico-docs` and
   the only permitted Graphiti group is `group_id=infolitico-docs`.
...
Flat MDX in `infolitico/` plus `docs.json` are canonical; `content/docs/` is
...
`infolitico/**` route prefix in canonical content and the rewrite list in
sync.
