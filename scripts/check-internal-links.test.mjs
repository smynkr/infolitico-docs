...
function fixture() {
  ...
  fs.mkdirSync(path.join(root, "infolitico"), { recursive: true });
  fs.mkdirSync(path.join(root, "public", "images"), { recursive: true });
    fs.writeFileSync(path.join(root, "infolitico", "index.mdx"), "# Layer\n\n## Setup\n\n## Setup\n\n[guide](/infolitico/guide)\n");
  fs.writeFileSync(path.join(root, "infolitico", "guide.mdx"), "---\ntitle: Guide\nfake: \"# frontmatter-heading\"\n---\n\n# Guide [#guide-start]\n\n## `risk_tier` thresholds\n\n## C++ — C#\n\n## [API overview](/infolitico/guide)\n\n## DELETE `/api/scheduled-exports?id=<uuid>`\n\n{/*\n## commented heading\n[ignored comment link](/missing-comment)\n*/}\n\n[setup](/infolitico#setup-1) [self](#guide-start) [underscore](#risk_tier-thresholds) [punctuation](#c--c) [linked heading](#api-overview) [inline code](#delete-apischeduled-exportsiduuid) [index](./index) [reference][guide-ref]\n\n[guide-ref]: /infolitico/guide#guide-start\n\n![logo](/images/logo.svg)\n\n<div href=\"/infolitico/guide#guide-start\" src=\"/images/logo.svg\" />\n<span href={\"/infolitico/guide#guide-start\"} />\n");
  fs.writeFileSync(path.join(root, "docs.json"), JSON.stringify({ navigation: { pages: ["infolitico/index", "infolitico/guide"] }, links: [{ href: "/infolitico/guide#guide-start" }, { href: "https://example.com" }] }));
...
test("resolves relative links from the served route instead of the source directory", () => {
  const root = fixture();
  const indexFile = path.join(root, "infolitico", "index.mdx");
  const guideFile = path.join(root, "infolitico", "guide.mdx");
  assert.equal(resolveTarget(root, indexFile, "./guide"), "/guide");
  assert.equal(resolveTarget(root, guideFile, "./index"), "/infolitico/index");
  assert.equal(resolveTarget(root, guideFile, "./index?tab=api#setup"), "/infolitico/index?tab=api#setup");
...
test("fails deterministically when canonical sources collide on one route", () => {
  const root = fixture();
  fs.mkdirSync(path.join(root, "infolitico", "guide"), { recursive: true });
  fs.writeFileSync(path.join(root, "infolitico", "guide", "index.mdx"), "# Nested guide\n");
  const result = check({ root });
  assert.match(
    result.failures.join("\n"),
    /duplicate canonical route \/infolitico\/guide: infolitico\/guide\.mdx, infolitico\/guide\/index\.mdx/,
  );
  assert.equal(result.routes.get("/infolitico/guide"), path.join(root, "infolitico", "guide.mdx"));
});

test("validates root fragments against the rewritten MenuWright index", () => {
  const root = fixture();
  fs.writeFileSync(path.join(root, "infolitico", "index.mdx"), "# Guide\n\n## Welcome aboard\n\n[welcome](/#welcome-aboard) [missing](/#not-a-heading)\n");
  const result = check({ root });
  assert.equal(result.routes.get("/"), path.join(root, "infolitico", "index.mdx"));
...
test("reports broken routes, fragments, navigation pages, and assets", () => {
  const root = fixture();
  fs.writeFileSync(path.join(root, "infolitico", "guide.mdx"), "# Guide\n\n[missing](/infolitico/nope) [bad fragment](/infolitico#nope) ![missing](/images/nope.svg)\n");
  fs.writeFileSync(path.join(root, "docs.json"), JSON.stringify({ navigation: { pages: ["infolitico/index", "infolitico/nope"] }, links: [{ href: "/infolitico/missing-href" }] }));
  const failures = check({ root }).failures.join("\n");
  assert.match(failures, /unresolved route \/infolitico\/nope/);
  assert.match(failures, /unresolved fragment \/infolitico#nope/);
  assert.match(failures, /missing public asset \/images\/nope\.svg/);
  assert.match(failures, /docs\.json: navigation route \/infolitico\/nope/);
  assert.match(failures, /unresolved route \/infolitico\/missing-href/);
  assert.match(failures, /guide\.mdx:3/);
});

test("does not treat fenced code comments as heading anchors", () => {
  const root = fixture();
  fs.appendFileSync(path.join(root, "infolitico", "guide.mdx"), "\n```md\n# fake-anchor\n[ignored fenced link](/missing-fenced)\n<a href=\"/missing-jsx-example\">example</a>\n```\n`[ignored inline link](/missing-inline)`\n[code comment](/infolitico/guide#fake-anchor)\n");
  const failures = check({ root }).failures.join("\n");
  assert.match(failures, /unresolved fragment \/infolitico\/guide#fake-anchor/);
...
test("validates static template literals and fails closed on dynamic JSX targets", () => {
  const root = fixture();
  fs.appendFileSync(
    path.join(root, "infolitico", "guide.mdx"),
    "\n<span href={`/infolitico/nope`} />\n<span href={route} />\n<img src={`/images/${assetName}.svg`} alt=\"Dynamic asset\" />\n",
  );
  const failures = check({ root }).failures.join("\n");
  assert.match(failures, /unresolved route \/infolitico\/nope/);
...
test("discovers page lists nested under new navigation containers", () => {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      navigation: {
        versions: [{ version: "v1", dropdowns: [{ dropdown: "Guides", pages: ["infolitico/nope"] }] }],
      },
    }),
  );
  ...
  assert.match(
    ...
    /docs\.json: navigation route \/infolitico\/nope has no canonical MDX source/,
  );
});
