...
export const sourceRoots = ["infolitico"];
...
export function check({ root = process.cwd() } = {}) {
  ...
  // Next rewrites `/` to `/menuwright` (the MenuWright index) without
  // replacing fragments, so root links must validate against that canonical
  // document's headings.
  const rootIndex = routeFiles.get("/getting-started") ?? routeFiles.get("/infolitico");
